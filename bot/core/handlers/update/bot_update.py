...
bot_status_router = Router()

@bot_status_router.my_chat_member(ChatMemberUpdatedFilter(member_status_changed=JOIN_TRANSITION))
async def bot_joined_to_channel(update: types.ChatMemberUpdated):
    """
    ...
    ** в текущем варианте подразумевается один инстанс бота на один канал
    """
    if config.channel_id != update.chat.id:
...
@bot_status_router.my_chat_member(ChatMemberUpdatedFilter(member_status_changed=LEAVE_TRANSITION))
async def bot_leave_from_channel(update: types.ChatMemberUpdated):
    """
