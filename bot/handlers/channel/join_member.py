...
async def join_request(update: types.ChatJoinRequest, repo: SQLAlchemyRepo, bot: Bot):
    """Заявка на вступление через основную ссылку-приглашение или ссылку, созданную одним из администраторов"""
    await DeclineChatJoinRequest(chat_id=update.chat.id, user_id=update.from_user.id)
    await bot.send_message(chat_id=update.from_user.id,
    ...
    loggers.event.info(
        ...
        f"была использована основная ссылка-приглашение или ссылка, созданная одним из администраторов")
