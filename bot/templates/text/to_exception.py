...
MESSAGE_IS_NOT_CONTACT = "Мне нужно, чтобы ты прислал(а) свой контакт нажатием кнопки <b>'Поделиться моим " \
                         "контактом'</b>\n\nДавай попробуем ещё раз\n\U000023EC\U000023EC\U000023EC"


async def request_is_found(username: str) -> str:
    return f"<b>{username}</b>, ты уже прошёл(ла) регистрацию ранее и получил своё приглашение в канал."


async def status_is_member(username: str) -> str:
    return f"<b>{username}</b>, ты уже подписан(а) на канал"


async def member_is_kicked(username: str) -> str:
    return f"<b>{username}</b>, ты заблокирован(а) в канале.\n\n" \
...
async def member_is_unknown(username: str) -> str:
    return f"<b>{username}</b>, ты пришёл(ла) в канал до того, как я появился на свет.\U0001F914 \n" \
